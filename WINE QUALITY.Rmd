...
```{r}
wine <- read.csv("C:\\Users\\victo\\OneDrive\\Documents\\Work\\1. ESSEC\\2. Master\\3. M2\\T1\\Business Data Science\\Project\\winequality.csv", header=T, stringsAsFactors = F)
View(wine)
str(wine)
sum(is.na(wine))
```
...
```{r echo=TRUE}
set.seed(300)
df <- read.csv("C:\\Users\\victo\\OneDrive\\Documents\\Work\\1. ESSEC\\2. Master\\3. M2\\T1\\Business Data Science\\Project\\winequality.csv", header=T, stringsAsFactors = F)
df$id <- NULL
x <- df[1:11]
y_num <- df$quality
y <- factor(df$quality)
